...
class CLRHead(nn.Module):
    def __init__(self,
        ...
        if iou_loss == 'lineiou':
            self.iou_loss = 'lineiou'
            self.iou_loss_func = liou_loss
        elif iou_loss == 'laneiou':
            self.iou_loss = 'laneiou'
            self.iou_loss_func = LaneIoULoss(loss_weight=1.0, lane_width=7.5 / 800, img_h=self.img_h, img_w=self.img_w)
    ...
    def predictions_to_pred(self, predictions):
        ...
        for lane in predictions:
            lane_xs = lane[6:]  # normalized values
    ...
    def loss(self,
        ...
        if self.cfg.haskey('cost_combination'):
            cost_combination = self.cfg.cost_combination
            ...
            for predictions, target in zip(predictions_list, targets):
                ...
                with torch.no_grad():
                    matched_row_inds, matched_col_inds = assign(
                        predictions, target, self.img_w, self.img_h, cost_combination=cost_combination)
                ...
                if self.iou_loss == 'lineiou':
                    iou_loss = iou_loss + self.iou_loss_func(
                        reg_pred, reg_targets,
                        self.img_w, length=15)
                elif self.iou_loss == 'laneiou':
                    iou_loss = iou_loss + self.iou_loss_func(reg_pred, reg_targets)
